  ...
  function ( CAP_NAMED_ARGUMENTS, group )
    ...
    morphism_constructor :=
      function ( SkeletalFinGSets, S, pair_of_lists, T )
        ...
        result := CreateCapCategoryMorphismWithAttributes( SkeletalFinGSets,
                       S,
                       T,
                          PairOfLists, pair_of_lists );
        Assert( 0, IsWellDefined( result ) );
    ...
    ##
    morphism_datum := { SkeletalFinGSets, phi } -> PairOfLists( phi );
    ...
    modeling_tower_morphism_constructor :=
      function( SkeletalFinGSets, source, pair_of_lists, target )
        local sFinGSets, TG, l, maps, mors, transitives,
    ...
    modeling_tower_morphism_datum :=
      function( SkeletalFinGSets, phi )
        local sFinGSets, TG, l, multiplicities_of_source, pair_of_lists;
...
InstallMethod( DisplayString,
        "for an object in a skeletal category finite G-set",
...
##
InstallMethod( DisplayString,
        "for a morphism in a skeletal category finite G-set",
