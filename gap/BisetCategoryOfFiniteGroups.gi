                      ...
                      List( [ 1 .. k ], o -> Pair(
                            ListWithIdenticalEntries( multiplicities[o], o - 1 ),
                                 Concatenation( List( [ 1 .. Length( k_pos[o] ) ], j ->
                                       Concatenation(
                                            List( [ 1 .. mult[k_pos[o][j]] ], c ->
                                                  perms[h][k_pos[o][j]] + (c - 1)*ltrs[k_pos[o][j]] - 1 + offsets[o][j] ) ) ) ) ) ) );
        mors := List( [ 1 .. lhg ], h ->
                      List( [ 1 .. k ], o -> 
                            Concatenation( List( [ 1 .. Length( k_pos[o] ) ],
                                  j -> Concatenation( ListWithIdenticalEntries( mult[ k_pos[o][j] ], data_mors[h][ k_pos[o][j] ] ) ) ) ) ) );
        ...
        phis := List( [ 1 .. lt ], i ->
          Subgroup( P,
            List( GeneratorsOfGroup( P1s[i] ), h ->
              Embedding(P,1)( h )
              * Embedding(P,2)( MorphismDatum( recomposition( action_pair, GroupAsCategoryMorphism( GroupAsCategory( H ), h ) ) )[2][ transitifs[i][1] ][ transitifs[i][2][1] ] ) 
              ) ) );
